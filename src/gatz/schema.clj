...
(def UserCRDT
  [:map
   ...
   [:user/settings [:map
                    ...
                    [:settings/location {:optional true} LocationPreferencesCRDT]]]
   [:user/profile [:map
                   [:profile/urls UserSettingsLinksCRDT]]]])

(def UserActivityCRDT
  [:map
   [:xt/id :uuid] ;; unused
   ...
   [:user_activity/last_active (crdt/max-wins-schema inst?)]
   [:user_activity/last_location (crdt/lww-schema [:maybe #'Location])]])
...
(def UserActivity
  [:map
   ...
   [:user_activity/last_location [:maybe #'Location]]])
...
(def DiscussionCRDT
  [:map
   ...
   [:discussion/location_id [:maybe string?]] ;; for the index
   [:discussion/location [:maybe #'Location]] ;; for rendering quickly
...
(def UserAction
  (mu/closed-schema
   [:or
    ...
    [:map
     [:gatz.crdt.user/action [:enum :gatz.crdt.user/update-location-settings]]
     [:gatz.crdt.user/delta UserUpdateLocationSettings]]
    [:map
...
(def FeedItemEvent
  [:map
   ...
   [:evt/uid #'UserId]
   [:evt/cid [:maybe #'ClientId]]
   [:evt/type [:enum :evt.feed_item/new]]
   [:evt/did [:maybe #'DiscussionId]]
   [:evt/feed_item #'FeedItemId]])

(def Event
  [:or #'DiscussionEvt #'MessageEvent #'ReactionEvt #'FeedItemEvent])
...
(def schema
  {:discussion/id #'DiscussionId
   ...
   :gatz/user_activity #'UserActivityCRDT
