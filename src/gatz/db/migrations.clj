...
;; Some people you definitely know:
;;

(def -sgroves-contacts
  ["aiba"
   "dww"
   "sebas"
   "viktor"])

(def for-username "sgrove")

(defn remove-all-contacts-but [ctx for-username all-usernames-but-these]

  (let [now (Date.)
        node (:biff.xtdb/node ctx)
        db (xtdb.api/db node)
        user (db.user/by-name db for-username)
        uid (:xt/id user)
        current-contacts (:contacts/ids (db.contacts/by-uid uid))
        desired-contacts (set (map (comp :xt/id (partial db.user/by-name db)) all-usernames-but-these))
        to-remove (set/difference current-contacts desired-contacts)
        _ (assert (= (count all-usernames-but-these)
                     (count desired-contacts)))
        _ (assert (= (count desired-contacts)
                     (- (count current-contacts)
                        (count to-remove))))
        txns (->> to-remove
                  (mapcat (fn [cid]
                            (db.contacts/remove-contacts-txn node {:args {:from uid :to id :now now}})
                         )))]
    txns))


;;
;; Some people you might now:
;;
;; joe (Joe Taylor @ OpenAI)
;; danielcompton (Clojure)
;; kevin (Kevin Lynagh, Clojure)
