You are an expert in TypeScript, jest, and test coverage analysis for TypeScript modules. Given a file with TypeScript functions and classes, you use test coverage reports to ensure comprehensive testing by identifying and covering all untested code paths.
...
- add debug logging or helper methods for testing
...
```sh
yarn test TEST_FILE -- --coverage --collectCoverageFrom=LOGIC_FILE
```
...
4. Can you see coverage data for LOGIC_FILE specifically?
</check-prompt>
</run-initial-coverage>

<analyze-coverage-report>
<action-prompt>
Use the coverage analysis script to automatically identify all uncovered code in LOGIC_FILE
Run the coverage analysis script and pipe its output to a file

```sh
$ yarn run analyze-coverage LOGIC_FILE TEST_FILES > tmp/coverage.initial.txt # via yarn
$ node bin/analyze-coverage.js LOGIC_FILE TEST_FILE > tmp/coverage.initial.txt # directly via node
```
...
Line 238: [coverage-function-7] Anonymous function never called
    const processData = useCallback(() => {
      return transformer.process(inputData);
...
1. Match uncovered areas to existing slugs: For each uncovered code snippet, check if there's already a matching slug annotation in LOGIC_FILE
2. Create new slugs for unmatched areas: If uncovered code doesn't have a corresponding slug, create a new one with the format [coverage-slug-name] and document it in the function's description in LOGIC_FILE
...
Change [coverage-function-7] to [data-processing-callback]
Change [coverage-branch-3] to [conditional-validation]
</action-prompt>
...
- Did you update the function descriptions in LOGIC_FILE with any new slugs you created?
...
// [index-bounds-check] Test the passed index is within the VALID_OPTIONS range
*/
```
...
```js
    // [validation-conditional] Test validation logic with different input types (Line 498)
    // - Test with valid and invalid inputs
    it('[validation-conditional] should handle different input validation scenarios', () => {
```

You can export additional functions or add debug helpers to any module you want. Avoid writing complex mocks when you can test the actual function behavior instead.
...
If it is ever not clear what to do next or what a command means, first re-read this file, .claude/commands/logic-coverage-tester.md. If that doesn't work, ask the user for help.
