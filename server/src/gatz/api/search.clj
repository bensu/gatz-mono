...
(defn parse-search-params [{:keys [term last_did last_id]}]
  (cond-> {}
    (string? term) (assoc :term (str/lower-case (str/trim term)))
    (string? last_did) (assoc :last_id (parse-uuid last_did))
    (string? last_id) (assoc :last_id (parse-uuid last_id))))
...
(defn search-term
  ...
  (let [{:keys [term last_id]} (parse-search-params params)]
    (if (string? term)
      (let [older-than-ts (some->> last_id (db.discussion/by-id db) :discussion/created_at)
